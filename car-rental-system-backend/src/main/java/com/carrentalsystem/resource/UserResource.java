...
public class UserResource {
	...
	public ResponseEntity<CommonApiResponse> registerAdmin(RegisterUserRequestDto registerRequest) {
		...
		if (registerRequest == null) {
			response.setResponseMessage("Invalid request - user cannot be empty.");
	...
	public ResponseEntity<CommonApiResponse> registerUser(RegisterUserRequestDto request) {
		...
		if (request.getRole() == null) {
			response.setResponseMessage("Invalid request - role information missing.");
			response.setSuccess(false);
		...
		if (savedAddress == null) {
			throw new UserSaveFailedException("Registration failed, a technical problem occurred.");
		}
	...
	public ResponseEntity<CommonApiResponse> updateUserStatus(UserStatusUpdateRequestDto request) {
		...
		if (request == null) {
			response.setResponseMessage("Invalid request - missing data.");
			response.setSuccess(false);
		...
		if (request.getUserId() == 0) {
			response.setResponseMessage("Invalid request - user ID missing.");
			response.setSuccess(false);
		...
		if (updatedUser == null) {
			throw new UserSaveFailedException("User status could not be updated.");
		}
	...
	public ResponseEntity<CommonApiResponse> addCustomerDrivingLicense(AddDrivingLicenseRequest request) {
		...
		if (addedLicense == null) {
			throw new UserSaveFailedException("Driving license could not be saved!");
		}
