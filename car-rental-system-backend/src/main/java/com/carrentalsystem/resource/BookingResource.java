...
public class BookingResource {
	...
	public ResponseEntity<CommonApiResponse> addBooking(AddBookingRequest request) {
		...
		if (customer == null) {
			response.setResponseMessage("Invalid request - customer not found.");
			response.setSuccess(false);
		...
		if (addedBooking == null) {
			throw new BookingSaveFailedException("Car rental process failed.");
		}

		response.setResponseMessage("Booking created successfully.");
	...
	public ResponseEntity<BookingResponse> fetchAllBookings() {
		...
		response.setResponseMessage("Bookings fetched successfully.");
		response.setSuccess(true);
	...
	public ResponseEntity<BookingResponse> fetchAllCustomerBookings(Integer customerId) {
		...
		if (customerId == null) {
			response.setResponseMessage("Invalid request - customer ID missing.");
			response.setSuccess(false);
	...
	public ResponseEntity<CommonApiResponse> updateStatusAndAssignVehicle(AddBookingRequest request) {
		...
		if (request.getStatus().equals(BookingStatus.APPROVED.value()) && request.getVehicleId() == null) {
			response.setResponseMessage("Please assign a vehicle for the booking.");
			response.setSuccess(false);
		...
		if (addedBooking == null) {
			throw new BookingSaveFailedException("Booking could not be updated.");
		}
	...
	public ResponseEntity<CommonApiResponse> customerPaymentForBooking(CustomerBookingPaymentRequest request) {
		...
		if (addedPayment == null) {
			throw new BookingSaveFailedException("Payment for the booking failed.");
		}
		...
		response.setResponseMessage("Payment successful, booking confirmed.");
		response.setSuccess(true);
	...
	public ResponseEntity<CommonApiResponse> cancelbooking(AddBookingRequest request) {
		...
		if (addedBooking == null) {
			throw new BookingSaveFailedException("Booking could not be cancelled.");
		}

		response.setResponseMessage("Booking cancelled successfully.");
		response.setSuccess(true);
